...
is made or until `beldexd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `beldex-wallet-cli` and
`beldex-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `beldexd` the ip/port of a socks proxy provided by a
...
which tells `beldexd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `beldexd` the inbound address, network
...
which tells `beldexd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `beldexd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `beldexd` localhost port 30000.
...
`beldexd` RPC port - which is independent from the configuration for incoming
...
```
HiddenServiceDir /var/lib/tor/data/beldex
...
This will store key information in `/var/lib/tor/data/beldex` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/beldex/hostname` will contain the ".onion" address for use
...
Run `beldexd` as often as possible to conceal when transactions are being sent.
...
`beldexd` currently selects two outgoing connections every 5 minutes for
...
`beldexd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `beldexd` should likely disconnect
