...
# - "mike the miner" is the miner and operator of the master nodes and so has an endlessly
...
class MNNetwork:
    def __init__(self, datadir, *, binpath='../../build/bin', sns=20, nodes=3):
        ...
        self.sns = [Daemon(master_node=True, **nodeopts) for _ in range(sns)]
        ...
        vprint("Waiting for all beldexd's to get ready")
        ...
        vprint("Fake MN network setup complete!")
...
def net(pytestconfig, tmp_path, binary_dir):
    """Fixture that returns the master node network.  It is persistent across tests: the first time
    it loads it starts the daemons and wallets, mines a bunch of blocks and submits MN
    ...
    if not snn:
        ...
        if verbose:
            print("\nConstructing initial master node network")
        snn = MNNetwork(datadir=tmp_path, binpath=binary_dir)
    else:
...
def chuck(net):
    ...
    hidden_node = Daemon(beldexd=net.binpath+'/beldexd', datadir=net.datadir)
    bridge_node = Daemon(beldexd=net.binpath+'/beldexd', datadir=net.datadir)
    for x in (4, 7):
        bridge_node.add_peer(net.all_nodes[x])
    bridge_node.add_peer(hidden_node)
    hidden_node.add_peer(bridge_node)

    vprint("Starting new chuck beldexd bridge node with RPC on {}:{}".format(bridge_node.listen_ip, bridge_node.rpc_port))
    bridge_node.start()
    bridge_node.wait_for_json_rpc("get_info")
    net.sync(extra_nodes=[bridge_node], extra_wallets=[chuck])

    vprint("Starting new chuck beldexd hidden node with RPC on {}:{}".format(hidden_node.listen_ip, hidden_node.rpc_port))
    hidden_node.start()
    ...
    chuck.refresh()
    chuck.stop()
    chuck.hidden = Wallet(node=hidden_node, name='Chuck (hidden)', rpc_wallet=net.binpath+'/beldex-wallet-rpc', datadir=net.datadir)
