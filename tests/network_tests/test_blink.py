...
from master_node_network import coins, vprint

def test_init(net):
    """Tests that the master node test network got initialized properly.  (This isn't really a test
    ...
    active_sns = net.nodes[0].json_rpc("get_n_master_nodes", dict(
    ...
    # We should have active flash quorums for the current height (minus flash quorum lag), and all
    ...
    assert len(flash_quorums) == len(net.all_nodes)                  # Got responses from every node
    assert flash_quorums == [flash_quorums[0]] * len(net.all_nodes)  # All nodes returned the same response
    assert len(flash_quorums[0]) == 2                                # Got both quorums
    assert tuple(len(q) for q in flash_quorums[0]) == (10, 10)       # Both quorum responses have full flash quorums
    ...
    # MN network is 1/(20*19*18*...*11) = 1.491552e-12):
    assert flash_quorums[0][0] != flash_quorums[0][1]
...
def test_basic_flash(net, alice, bob, mike):
    """Tests sending a basic flash (alongside a non-flash tx) and verifies the it is accepted and
    ...
    sent_flash = alice.transfer(bob, coins(10), priority=5)
    assert sent_flash['amount_list'] == [coins(10)]
    assert len(sent_flash['tx_hash_list']) == 1
    ...
    recv_flash, recv_normal = bob.find_transfers(x['tx_hash_list'][0] for x in (sent_flash, sent_normal))
    assert recv_flash is not None
    assert recv_normal is not None

    exp_flash  = {'type': 'in',   'address': bob.address(), 'amount': coins(10), 'flash_mempool': True,  'was_flash': True}
    exp_normal = {'type': 'pool', 'address': bob.address(), 'amount': coins(3),  'flash_mempool': False, 'was_flash': False}
    assert exp_flash.items() <= recv_flash.items()
    assert exp_normal.items() <= recv_normal.items()
    assert diff(b1, bob.balances()) == coins(10, 0)  # Flash balance shows up now, mempool after being mined.

    net.mine(1, sync=True)

    assert diff(b1, bob.balances()) == coins(13, 0)
    recv_flash, recv_normal = bob.find_transfers(x['tx_hash_list'][0] for x in (sent_flash, sent_normal))
    exp_flash['flash_mempool'] = False
    assert exp_flash.items() <= recv_flash.items()
    exp_normal['type'] = 'in'
    assert exp_normal.items() <= recv_normal.items()
...
def test_flash_sweep(net, mike, alice, bob):
    """Similar to the above, but tests flashing a sweep_all"""
    ...
    # huge one that requires multiple txes that alice flashes to bob.
    #
...
def test_flash_fail_mempool(net, mike, alice, bob):
    """This test does a regular transfer then attempts to flash the same funds via a local node with
    a flushed txpool; the flash should get rejected by the quorum."""
    ...
    # A non-flash should go through, since it only validates on the local node, and we just clear
    # its mempool:
    alice.transfer(bob, coins(7))

    # Clear it again, then try a flash:
    assert alice.node.json_rpc("flush_txpool").json()['result']['status'] == 'OK'
    assert 'result' in alice.json_rpc("rescan_spent").json()
...
def test_flash_replacement(net, mike, alice, chuck, chuck_double_spend):
    """Tests that a conflicting non-flash tx on a node gets replaced by the arrival of a
    flash tx that conflicts with it.
    ...
    *only* connects to the bridge node):
    ...
    We then submit, from two copies of the same wallet, a double spend: a flash on the regular
    network and a non-flash on the hidden node.  The regular network accepts the flash, and the
    hidden node accepts the non-flash into its mempool.
    ...
    does, it also gets deleted by the flash arrival).
    """

    flash_hash, hidden_hash = (x['tx_hash_list'][0] for x in chuck_double_spend)

    # Current state:
    # - all the regular network nodes have the flash tx
    ...
    # that the hidden tx is gone from everywhere and just the flash remains.
    ...
    assert chuck.bridge.txpool_hashes() == [flash_hash]
    assert chuck.hidden.node.txpool_hashes() == [flash_hash]
    assert alice.node.txpool_hashes() == [flash_hash]


@pytest.mark.parametrize('rollback_size', (1, 5))
def test_flash_rollback(net, mike, alice, chuck, chuck_double_spend, rollback_size):
    """Tests that a mined, conflicting non-flash tx on a node gets rolled back by the arrival of a
    flash tx that conflicts with it.
    ...
    # has the conflicting block and will get the flash from the bridge node.
    chuck.hidden.node.p2p_resync()
    time.sleep(1)
    ...
    assert chuck.hidden.node.txpool_hashes() == [flash_hash]
    assert alice.node.txpool_hashes() == [flash_hash]
    assert [x.txpool_hashes() for x in net.all_nodes] == [[flash_hash] for _ in net.all_nodes]
