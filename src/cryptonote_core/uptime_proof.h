...
public:
  ...
  Proof() = default;
  Proof(uint32_t mn_public_ip, uint16_t mn_storage_https_port, uint16_t mn_storage_omq_port, std::array<uint16_t, 3> ss_version, uint16_t quorumnet_port, std::array<uint16_t, 3> beldexnet_version, const master_nodes::master_node_keys& keys);
